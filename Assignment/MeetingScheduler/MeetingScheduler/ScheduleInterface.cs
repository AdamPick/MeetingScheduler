...
namespace MeetingScheduler
{
    public partial class ScheduleInterface : Form
    {
        ...
        {
            string path = System.AppDomain.CurrentDomain.BaseDirectory;
            string filePath = path.Substring(0, path.Length - ("\\bin\\Debug").Length) + fileName;
        ...
        private void ParticipantsAvailable_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (addParticipant == true)
                if (participantsSelected.Count() > 0)
                {
                    ...
                    {
                        if (ParticipantsAvailable.SelectedItem.ToString() == person)
                        {
                            found = true;
                            break;
                        }
                    ...
                    if (found == false)
                    {
                        participantsSelected.Add(ParticipantsAvailable.SelectedValue.ToString());
                        numPeopleSelected++;
                        DisplayCurrentDetails();
                    }
                }
            ...
            else
            {
                if (participantsSelected.Count() > 0)
                {
                    participantsSelected.Remove(ParticipantsAvailable.SelectedValue.ToString());
                    numPeopleSelected--;
                    DisplayCurrentDetails();
                }
            }
